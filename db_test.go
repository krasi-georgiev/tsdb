...
func TestDB_reloadOrder(t *testing.T) {
	...
	for _, m := range metas {
		createBlock(t, db.Dir(), genSeries(1, 1, m.MinTime, m.MaxTime, true))
	}
...
func TestTombstoneCleanFail(t *testing.T) {
	...
	for i := 0; i < totalBlocks; i++ {
		blockDir := createBlock(t, db.Dir(), genSeries(1, 1, 0, 0, true))
...
func (c *mockCompactorFailing) Write(dest string, b BlockReader, mint, maxt int64, parent *BlockMeta) (ulid.ULID, error) {
	...
	block, err := OpenBlock(nil, createBlock(c.t, dest, genSeries(1, 1, 0, 0, true)), nil)
	testutil.Ok(c.t, err)
...
func TestTimeRetention(t *testing.T) {
	...
	for _, m := range blocks {
		createBlock(t, db.Dir(), genSeries(10, 10, m.MinTime, m.MaxTime, true))
	}
...
func TestSizeRetention(t *testing.T) {
	...
	for _, m := range blocks {
		createBlock(t, db.Dir(), genSeries(100, 10, m.MinTime, m.MaxTime, true))
	}
...
func TestInitializeHeadTimestamp(t *testing.T) {
	...
	t.Run("existing-block", func(t *testing.T) {
		...
		createBlock(t, dir, genSeries(1, 1, 1000, 2000, true))
	...
	t.Run("existing-block-and-wal", func(t *testing.T) {
		...
		createBlock(t, dir, genSeries(1, 1, 1000, 6000, true))
...
func TestNoEmptyBlocks(t *testing.T) {
	...
	t.Run("Test no blocks remaining after deleting all samples from disk.", func(t *testing.T) {
		...
		for _, m := range blocks {
			createBlock(t, db.Dir(), genSeries(2, 2, m.MinTime, m.MaxTime, true))
		}
...
func TestBlockRanges(t *testing.T) {
	...
	createBlock(t, dir, genSeries(1, 1, 0, firstBlockMaxT, true))
	...
	createBlock(t, dir, genSeries(1, 1, secondBlockMaxt+1, thirdBlockMaxt, true))
